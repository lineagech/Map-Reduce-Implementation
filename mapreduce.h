...
struct map_reduce {
    ...
    map_fn       map;
    reduce_fn    reduce;
    int          num_threads;       // number of threads, set by mr_create
    pthread_t    *threads_arr;      // map
    pthread_t    reduce_thread;     // reduce
    int          buffer_size;       // each thread's buffer maximum size set by mr_create
    char         **buffer_arr;      // each thread's buffer, e.g. buffer_arr[0] is for thread 0 
    int          *threads_arg_arr;  // argument passing to each thread 
    int          *curr_buf_size;    // current buffer size generated by mapper and consumed by reducer

    pthread_mutex_t *mutex_arr;
    pthread_cond_t *buf_avail;
    int          *exec_done;
    int 	 *infds;
};
